...
Begin VB.Form FReceivable 
   ...
   Begin VB.CommandButton CNew 
      Height          =   505
      Left            =   285
      Picture         =   "FReceivable.frx":1FEC42
      Style           =   1  'Graphical
      TabIndex        =   6
      Top             =   4005
      Width           =   1365
   ...
   Begin VB.CommandButton CSave 
      Height          =   505
      Left            =   3210
      Picture         =   "FReceivable.frx":2010A4
      Style           =   1  'Graphical
      TabIndex        =   5
      Top             =   4005
      Width           =   1365
   ...
   Begin VB.CommandButton CClose 
      ...
      Height          =   505
      Left            =   4665
      Picture         =   "FReceivable.frx":203506
      Style           =   1  'Graphical
      TabIndex        =   8
      Top             =   4005
      Width           =   1365
   ...
   Begin VB.CommandButton CDelete 
      Height          =   505
      Left            =   3990
      Picture         =   "FReceivable.frx":205968
      Style           =   1  'Graphical
      TabIndex        =   7
      Top             =   165
      Width           =   1365
...
Dim rs As Recordset
    ...
    Set rs = db.OpenRecordset("Select AccountTransaction.* From AccountTransaction Where (AccountTransaction.BillNo = '" & Trim(TVoucherNo.Text) & "' ) And (AccountTransaction.Type = 'RV' ) And (AccountTransaction.FinancialCode='" & getFinancialCode(DTPDate.Value) & "')")
...
Dim sStatus As String
    ...
    rs!CreditedDebitedTo = "Receivable"
    rs!Mode = "Credit"
    rs.Update
...
Dim rs As Recordset, sTransactionNo As String
    ...
    Set rs = db.OpenRecordset("Select Max(Val(AccountTransaction.BillNo)) As TNo From AccountTransaction Where (AccountTransaction.Type = 'RV')  And (AccountTransaction.FinancialCode='" & getFinancialCode(DTPDate.Value) & "')")
...
Dim rs As Recordset
    ...
    clearControls
    Set rs = db.OpenRecordset("Select AccountRegister.AccountName,AccountTransaction.AddedDate,AccountTransaction.Debit,AccountTransaction.Narration From AccountRegister,AccountTransaction Where (AccountRegister.Code = AccountTransaction.AccountCode) And (AccountTransaction.BillNo = '" & Trim(sTransactionNo) & "' ) And (AccountTransaction.Type = 'RV' ) And (AccountTransaction.FinancialCode='" & getFinancialCode(DTPDate.Value) & "')")
    If rs.RecordCount > 0 Then
