...
Dim sSupplierCode() As String, sSupplierAddress() As String, sAccountCode() As String, sTin() As String
...
Dim rs As Recordset, sTransactionNo As String
    ...
    Set rs = db.OpenRecordset("Select Max(Val( Transaction.BillNo)) As TNo From Transaction Where ( Transaction.BillType = 'P' ) And (Transaction.FinancialCode='" & getFinancialCode(DTPDate.Value) & "')")
...
Dim rs As Recordset
    ...
    Set rs = db.OpenRecordset("Select SupplierMaster.SupplierCode,SupplierMaster.AccountCode,SupplierMaster.SupplierName,SupplierMaster.Address1,SupplierMaster.Address2,SupplierMaster.Address3,SupplierMaster.TinNo From SupplierMaster Where (SupplierMaster.Status = True) Order By SupplierMaster.SupplierName")
    ...
    While rs.EOF = False
        ...
        sTin(CoSupplier.ListCount) = "" & rs!TinNo
...
Dim rs As Recordset, lYN As Long, bFound As Boolean
    ...
    If (MsgBox("Do you want to Delete the Bill ?", vbDefaultButton2 Or vbYesNo) = vbYes) Then
        Set rs = db.OpenRecordset("Select Transaction.* From Transaction Where (Transaction.BillNo = '" & Trim(TTransactionNo.Text) & "' ) And (Transaction.BillType = 'P' ) And (Transaction.FinancialCode='" & getFinancialCode(DTPDate.Value) & "')")
        While rs.EOF = False
            bFound = True
            rs.Delete
            rs.MoveNext
...
Private Sub CoSupplier_Change()
    If CoSupplier.ListIndex <> -1 Then
        ...
        LTin.Caption = sTin(CoSupplier.ListIndex + 1)
    Else
        TAddress.Text = ""
        LTin.Caption = ""
    End If
...
Dim rs As Recordset, r As Long
    ...
    Set rs = db.OpenRecordset("Select ItemRegister.ItemName,ItemRegister.BillingName,ItemRegister.Code,Units.UnitName,Transaction.*,(Select IM.ItemName From ItemRegister AS IM Where(IM.Code=ItemRegister.GroupCode)) As GroupName From ItemRegister,Transaction,Units Where (Transaction.BillNo = '" & Trim(TTransactionNo.Text) & "' ) And (Transaction.BillType = 'P' ) And (ItemRegister.Code = Transaction.ItemCode ) And (Units.Code = ItemRegister.PurchaseUnitCode ) And (Transaction.FinancialCode='" & getFinancialCode(DTPDate.Value) & "') Order By Transaction.SerialNo")
    MGrid.Rows = 0
    If rs.RecordCount > 0 Then
