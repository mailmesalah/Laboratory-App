...
Begin VB.Form FSalesForm8Report 
   ...
   EndProperty
   Icon            =   "FSalesForm8Report.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MinButton       =   0   'False
   Picture         =   "FSalesForm8Report.frx":000C
   ScaleHeight     =   6780
   ScaleWidth      =   15240
   ...
   Begin VB.CommandButton CShowDetailed 
      Height          =   505
      Left            =   1815
      Picture         =   "FSalesForm8Report.frx":1FEC4E
      Style           =   1  'Graphical
      TabIndex        =   6
      Top             =   6150
      Width           =   1350
   ...
   Begin VB.CommandButton CShowSummary 
      Height          =   505
      Left            =   375
      Picture         =   "FSalesForm8Report.frx":2010B0
      Style           =   1  'Graphical
      TabIndex        =   5
      Top             =   6150
      Width           =   1350
   ...
   Begin VB.CommandButton CToExcel 
      Height          =   525
      Left            =   3225
      Picture         =   "FSalesForm8Report.frx":203512
      Style           =   1  'Graphical
      TabIndex        =   7
      Top             =   6135
      Width           =   1365
   ...
   Begin VB.CommandButton CClose 
      ...
      Height          =   505
      Left            =   13605
      Picture         =   "FSalesForm8Report.frx":205974
      Style           =   1  'Graphical
      TabIndex        =   8
      Top             =   6120
      Width           =   1365
   ...
   End
End
Attribute VB_Name = "FSalesForm8Report"
...
Dim rs As Recordset
    ...
    If CoItem.ListIndex = -1 And CoCustomer.ListIndex = -1 Then
        If CoManufacturer.ListIndex = -1 Then
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName From Transaction,ItemRegister Where (BillType = 'S8' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (ItemRegister.Code=Transaction.ItemCode) Group By BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName Order By BillDate,Val(BillNo)")
        Else
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName From Transaction,ItemRegister Where (BillType = 'S8' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (ItemRegister.Code=Transaction.ItemCode) And (Transaction.ItemCode In (Select ItemRegister.Code From ItemRegister Where(ItemRegister.ManufacturerCode='" & sManufacturerCode(CoManufacturer.ListIndex + 1) & "'))) Group By BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName Order By BillDate,Val(BillNo)")
        End If
    ElseIf CoItem.ListIndex = -1 And CoCustomer.ListIndex > -1 Then
        If CoManufacturer.ListIndex = -1 Then
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName From Transaction,ItemRegister Where (BillType = 'S8' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (ItemRegister.Code=Transaction.ItemCode) And (Transaction.CustomerCode ='" & sCustomerCode(CoCustomer.ListIndex + 1) & "') Group By BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName Order By BillDate,Val(BillNo)")
        Else
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName From Transaction,ItemRegister Where (BillType = 'S8' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (ItemRegister.Code=Transaction.ItemCode) And (Transaction.ItemCode In (Select ItemRegister.Code From ItemRegister Where(ItemRegister.ManufacturerCode='" & sManufacturerCode(CoManufacturer.ListIndex + 1) & "'))) And (Transaction.CustomerCode ='" & sCustomerCode(CoCustomer.ListIndex + 1) & "') Group By BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName Order By BillDate,Val(BillNo)")
        End If
    ElseIf CoItem.ListIndex > -1 And CoCustomer.ListIndex = -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName From Transaction,ItemRegister Where (BillType = 'S8' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (ItemRegister.Code=Transaction.ItemCode) And (Transaction.ItemCode ='" & sItemCode(CoItem.ListIndex + 1) & "') Group By BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName Order By BillDate,Val(BillNo)")
    ElseIf CoItem.ListIndex > -1 And CoCustomer.ListIndex > -1 Then
...
Dim rs As Recordset
    ...
    Me.Caption = "Sales Form 8 Report - Summary"
    ...
    If CoItem.ListIndex = -1 And CoCustomer.ListIndex = -1 Then
        If CoManufacturer.ListIndex = -1 Then
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.SaleRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.SaleRate) As TotalAmount From Transaction Where (BillType = 'S8' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) Group By BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
        Else
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.SaleRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.SaleRate) As TotalAmount From Transaction Where (BillType = 'S8' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) And (Transaction.ItemCode In (Select ItemRegister.Code From ItemRegister Where(ItemRegister.ManufacturerCode='" & sManufacturerCode(CoManufacturer.ListIndex + 1) & "'))) Group By BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
        End If
    ElseIf CoItem.ListIndex = -1 And CoCustomer.ListIndex > -1 Then
        
        If CoManufacturer.ListIndex = -1 Then
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.SaleRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.SaleRate) As TotalAmount From Transaction Where (BillType = 'S8' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) And (Transaction.CustomerCode ='" & sCustomerCode(CoCustomer.ListIndex + 1) & "') Group By BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
        Else
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.SaleRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.SaleRate) As TotalAmount From Transaction Where (BillType = 'S8' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) And (Transaction.ItemCode In (Select ItemRegister.Code From ItemRegister Where(ItemRegister.ManufacturerCode='" & sManufacturerCode(CoManufacturer.ListIndex + 1) & "'))) And (Transaction.CustomerCode ='" & sCustomerCode(CoCustomer.ListIndex + 1) & "') Group By BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
        End If
     
    ElseIf CoItem.ListIndex > -1 And CoCustomer.ListIndex = -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.SaleRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.SaleRate) As TotalAmount From Transaction Where (BillType = 'S8' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) And (Transaction.ItemCode ='" & sItemCode(CoItem.ListIndex + 1) & "') Group By BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
    ElseIf CoItem.ListIndex > -1 And CoCustomer.ListIndex > -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.SaleRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.SaleRate) As TotalAmount From Transaction Where (BillType = 'S8' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) And (Transaction.ItemCode ='" & sItemCode(CoItem.ListIndex + 1) & "') And (Transaction.CustomerCode ='" & sCustomerCode(CoCustomer.ListIndex + 1) & "') Group By BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
    End If
