...
Begin VB.Form FPurchaseReturnReport 
   ...
   EndProperty
   Icon            =   "FPurchaseReturnReport.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MinButton       =   0   'False
   Picture         =   "FPurchaseReturnReport.frx":000C
   ScaleHeight     =   6810
   ScaleWidth      =   15270
   ...
   Begin VB.CommandButton CShowDetailed 
      Height          =   505
      Left            =   1815
      Picture         =   "FPurchaseReturnReport.frx":1FEC4E
      Style           =   1  'Graphical
      TabIndex        =   4
      Top             =   6150
      Width           =   1350
   ...
   Begin VB.CommandButton CShowSummary 
      Height          =   505
      Left            =   375
      Picture         =   "FPurchaseReturnReport.frx":2010B0
      Style           =   1  'Graphical
      TabIndex        =   3
      Top             =   6150
      Width           =   1350
   ...
   Begin VB.CommandButton CToExcel 
      Height          =   525
      Left            =   3225
      Picture         =   "FPurchaseReturnReport.frx":203512
      Style           =   1  'Graphical
      TabIndex        =   5
      Top             =   6135
      Width           =   1365
   ...
   Begin VB.CommandButton CClose 
      ...
      Height          =   505
      Left            =   13500
      Picture         =   "FPurchaseReturnReport.frx":205974
      Style           =   1  'Graphical
      TabIndex        =   6
      Top             =   6120
      Width           =   1365
   ...
   End
End
Attribute VB_Name = "FPurchaseReturnReport"
...
Dim rs As Recordset
    ...
    Me.Caption = "Purchase Return Report - Detailed"
    
    Dim dQuantity, dGrossValue, dTaxAmount, dTotalAmount As Double

    'Set rs = db.OpenRecordset("Select BillDate,BillNo,Supplier,Tax,Transaction.Quantity,Transaction.PurchaseRate,ItemName From Transaction,ItemRegister Where (BillType = 'PR' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (ItemRegister.Code=Transaction.ItemCode) Group By BillDate,BillNo,Supplier,Tax,Transaction.Quantity,Transaction.PurchaseRate,ItemName Order By BillDate,Val(BillNo)")
    If CoItem.ListIndex = -1 And CoSupplier.ListIndex = -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Supplier,Tax,Transaction.Quantity,Transaction.PurchaseRate,ItemName From Transaction,ItemRegister Where (BillType = 'PR' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (ItemRegister.Code=Transaction.ItemCode) Group By BillDate,BillNo,Supplier,Tax,Transaction.Quantity,Transaction.PurchaseRate,ItemName Order By BillDate,Val(BillNo)")
    ElseIf CoItem.ListIndex = -1 And CoSupplier.ListIndex > -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Supplier,Tax,Transaction.Quantity,Transaction.PurchaseRate,ItemName From Transaction,ItemRegister Where (BillType = 'PR' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (ItemRegister.Code=Transaction.ItemCode) And (Transaction.SupplierCode ='" & sSupplierCode(CoSupplier.ListIndex + 1) & "') Group By BillDate,BillNo,Supplier,Tax,Transaction.Quantity,Transaction.PurchaseRate,ItemName Order By BillDate,Val(BillNo)")
    ElseIf CoItem.ListIndex > -1 And CoSupplier.ListIndex = -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Supplier,Tax,Transaction.Quantity,Transaction.PurchaseRate,ItemName From Transaction,ItemRegister Where (BillType = 'PR' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "'))  And(Transaction.ItemCode ='" & sItemCode(CoItem.ListIndex + 1) & "') And (ItemRegister.Code=Transaction.ItemCode) Group By BillDate,BillNo,Supplier,Tax,Transaction.Quantity,Transaction.PurchaseRate,ItemName Order By BillDate,Val(BillNo)")
    ElseIf CoItem.ListIndex > -1 And CoSupplier.ListIndex > -1 Then
...
Dim rs As Recordset
    ...
    Me.Caption = "Purchase Return Report - Summary"
    
    Dim dAdvance, dDiscount, dExtraCharge, dTaxAmount, dBasicAmount, dBillAmount, dBalance As Double
    
    'Set rs = db.OpenRecordset("Select BillDate,BillNo,Supplier,Narration,Advance,Discount,ExtraCharges,Tax,Sum(Abs(Transaction.Quantity)*Transaction.PurchaseRate) As TotalAmount From Transaction Where (BillType = 'PR' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) Group By BillDate,BillNo,Supplier,Narration,Advance,Discount,ExtraCharges,Tax Order By BillDate,Val(BillNo)")
    If CoItem.ListIndex = -1 And CoSupplier.ListIndex = -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Supplier,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.PurchaseRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.PurchaseRate) As TotalAmount From Transaction Where (BillType = 'PR' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) Group By BillDate,BillNo,Supplier,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
    ElseIf CoItem.ListIndex = -1 And CoSupplier.ListIndex > -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Supplier,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.PurchaseRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.PurchaseRate) As TotalAmount From Transaction Where (BillType = 'PR' ) And(SupplierCode ='" & sSupplierCode(CoSupplier.ListIndex + 1) & "') And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) Group By BillDate,BillNo,Supplier,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
    ElseIf CoItem.ListIndex > -1 And CoSupplier.ListIndex = -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Supplier,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.PurchaseRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.PurchaseRate) As TotalAmount From Transaction Where (BillType = 'PR' )And(Transaction.ItemCode ='" & sItemCode(CoItem.ListIndex + 1) & "') And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) Group By BillDate,BillNo,Supplier,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
    ElseIf CoItem.ListIndex > -1 And CoSupplier.ListIndex > -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Supplier,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.PurchaseRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.PurchaseRate) As TotalAmount From Transaction Where (BillType = 'PR' ) And(Transaction.ItemCode ='" & sItemCode(CoItem.ListIndex + 1) & "') And (Transaction.SupplierCode ='" & sSupplierCode(CoSupplier.ListIndex + 1) & "') And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) Group By BillDate,BillNo,Supplier,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
    End If
...
Private Sub MGrid_DblClick()
    If MGrid.Rows > 0 Then
        ...
        FPurchaseReturn.TTransactionNo = MGrid.TextMatrix(MGrid.Row, 1)
