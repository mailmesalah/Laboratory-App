...
Begin VB.Form FBillReport 
   ...
   EndProperty
   Icon            =   "FBillReport.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MinButton       =   0   'False
   Picture         =   "FBillReport.frx":000C
   ScaleHeight     =   8040
   ScaleWidth      =   15270
   ...
   Begin VB.CommandButton CShowSummary 
      Height          =   505
      Left            =   375
      Picture         =   "FBillReport.frx":1FEC4E
      Style           =   1  'Graphical
      TabIndex        =   19
      Top             =   7320
      Width           =   1350
   ...
   Begin VB.CommandButton CShowDetailed 
      Height          =   505
      Left            =   1815
      Picture         =   "FBillReport.frx":2010B0
      Style           =   1  'Graphical
      TabIndex        =   7
      Top             =   7320
      Width           =   1350
   ...
   Begin VB.CommandButton CToExcel 
      Height          =   525
      Left            =   3225
      Picture         =   "FBillReport.frx":203512
      Style           =   1  'Graphical
      TabIndex        =   8
      Top             =   7305
      Width           =   1365
   ...
   Begin VB.CommandButton CClose 
      ...
      Height          =   505
      Left            =   13605
      Picture         =   "FBillReport.frx":205974
      Style           =   1  'Graphical
      TabIndex        =   9
      Top             =   7290
      Width           =   1365
...
Dim gDate As Single, gBillNo As Single, gDoctor As Single, gPatient As Single, gAge As Single, gSex As Single, gEmail As Single, gMobile As Single, gDepartment As Single, gTest As Single, gSubTest As Single, gCost As Single, gStdCost As Single, gDiscount As Single
Dim rs As Recordset
    ...
    MGrid.ColWidth(gStdCost) = 1500
    MGrid.ColWidth(gDiscount) = 1500
    MGrid.ColWidth(gCost) = 1500
    ...
    MGrid.CellFontName = "Cosmic San"
    MGrid.CellFontSize = 10
    MGrid.TextMatrix(0, gStdCost) = "Standard Cost"
    ...
    MGrid.CellFontName = "Cosmic San"
    MGrid.CellFontSize = 10
    MGrid.TextMatrix(0, gCost) = "Cost"

    MGrid.RowHeightMin = 350

    'SHOW DATAS ON GRID

    Me.Caption = "Bill Report Detailed"
    
    Set rs = db.OpenRecordset("Select Transaction.BillDate , Transaction.BillNo, DoctorMaster.DoctorName, Transaction.Patient, Transaction.Age, Transaction.Sex, Transaction.email, Transaction.Mobile, Department.DepartmentName,T1.TestName As Test,T2.TestName As SubTest,T2.DefaultValue,T2.Cost As StdCost,Units.UnitName,Transaction.Cost From Transaction,DoctorMaster,Department,Units,TestRegister As T1,TestRegister As T2 Where (Transaction.BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (DoctorMaster.DoctorCode=Transaction.DoctorCode) And (Units.Code=T2.UnitCode) And (Department.DepartmentCode=T1.DepartmentCode) And (T1.Code=Transaction.TestCode) And (T2.Code=Transaction.SubTestCode) " _
    ...
    Dim total As Double, discount As Double, stdCost As Double
    total = 0
    While rs.EOF = False
        stdCost = stdCost + Val("" & rs!stdCost)
        discount = discount + (Val("" & rs!stdCost) - Val("" & rs!Cost))
        total = total + Val("" & rs!Cost)
        MGrid.AddItem Format("" & rs!BillDate, "dd-MM-yyyy") & vbTab & "" & rs!BillNo & vbTab & "" & rs!DoctorName & vbTab & rs!Patient & vbTab & rs!Age & vbTab & rs!Sex & vbTab & rs!email & vbTab & rs!Mobile & vbTab & rs!DepartmentName & vbTab & rs!Test & vbTab & rs!SubTest & vbTab & Format(Val("" & rs!stdCost), "0.00") & vbTab & Format(Val("" & rs!stdCost) - Val("" & rs!Cost), "0.00") & vbTab & Format(Val("" & rs!Cost), "0.00")
        rs.MoveNext
    Wend
    MGrid.AddItem "" & vbTab & "" & vbTab & "" & vbTab & vbTab & vbTab & vbTab & vbTab & vbTab & vbTab & vbTab & "Total" & vbTab & Format(stdCost, "0.00") & vbTab & Format(discount, "0.00") & vbTab & Format(total, "0.00")
    rs.Close
...
Dim gDate As Single, gBillNo As Single, gDoctor As Single, gPatient As Single, gAge As Single, gSex As Single, gEmail As Single, gMobile As Single, gCost As Single, gStdCost As Single, gDiscount As Single
Dim rs As Recordset
    ...
    MGrid.CellFontName = "Cosmic San"
    MGrid.CellFontSize = 10
    MGrid.TextMatrix(0, gCost) = "Bill Amount"

    MGrid.RowHeightMin = 350

    'SHOW DATAS ON GRID

    Me.Caption = "Bill Report Summary"
    
    Set rs = db.OpenRecordset("Select Transaction.BillDate , Transaction.BillNo, DoctorMaster.DoctorName, Transaction.Patient, Transaction.Age, Transaction.Sex, Transaction.email, Transaction.Mobile,Sum(Transaction.Cost) As Cost,Sum(T2.Cost) As StdCost From Transaction,DoctorMaster,TestRegister As T1,TestRegister As T2  Where (Transaction.BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (DoctorMaster.DoctorCode=Transaction.DoctorCode) And (T1.Code=Transaction.TestCode) And (T2.Code=Transaction.SubTestCode) " _
    & IIf(CoDoctor.ListIndex = -1, "", " And (Transaction.DoctorCode='" & sDoctorCode(CoDoctor.ListIndex + 1) & "') ") _
    & IIf(CoTest.ListIndex = -1, "", " And (Transaction.TestCode='" & sTestCode(CoTest.ListIndex + 1) & "') ") _
    & IIf(CoSubTest.ListIndex = -1, "", " And (Transaction.SubTestCode='" & sSubTestCode(CoSubTest.ListIndex + 1) & "') ") _
    & IIf(CoDepartment.ListIndex = -1, "", " And (T1.DepartmentCode='" & sDepartmentCode(CoDepartment.ListIndex + 1) & "') ") _
    ...
    & " Or (Transaction.email Like '" & Trim(TPatient.Text) & "') " _
    & " Or (Transaction.Mobile Like '" & Trim(TPatient.Text) & "')) ") _
    & " Group By Transaction.BillDate,Transaction.BillNo, DoctorMaster.DoctorName, Transaction.Patient, Transaction.Age,Transaction.email, Transaction.Sex, Transaction.Mobile " _
    & " Order By Transaction.BillDate,Val(Transaction.BillNo)")
    Dim total As Double, stdCost As Double, discount As Double
    total = 0
    stdCost = 0
    discount = 0
    While rs.EOF = False
        ...
        stdCost = stdCost + Val("" & rs!stdCost)
        discount = discount + (Val("" & rs!stdCost) - Val("" & rs!Cost))
        MGrid.AddItem Format("" & rs!BillDate, "dd-MM-yyyy") & vbTab & "" & rs!BillNo & vbTab & "" & rs!DoctorName & vbTab & rs!Patient & vbTab & rs!Age & vbTab & rs!Sex & vbTab & rs!email & vbTab & rs!Mobile & vbTab & Format(Val("" & rs!stdCost), "0.00") & vbTab & Format(Val("" & rs!stdCost) - Val("" & rs!Cost), "0.00") & vbTab & Format(Val("" & rs!Cost), "0.00")
        rs.MoveNext
    Wend
    MGrid.AddItem "" & vbTab & "" & vbTab & "" & vbTab & vbTab & vbTab & vbTab & vbTab & "Total" & vbTab & Format(stdCost, "0.00") & vbTab & Format(discount, "0.00") & vbTab & Format(total, "0.00")
    rs.Close
