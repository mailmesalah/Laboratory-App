...
Begin VB.Form FPurchaseReport 
   ...
   EndProperty
   Icon            =   "FPurchaseReport.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MinButton       =   0   'False
   Picture         =   "FPurchaseReport.frx":000C
   ScaleHeight     =   6810
   ScaleWidth      =   15270
   ...
   Begin VB.CommandButton CClose 
      ...
      Height          =   505
      Left            =   13530
      Picture         =   "FPurchaseReport.frx":1FEC4E
      Style           =   1  'Graphical
      TabIndex        =   8
      Top             =   6180
      Width           =   1365
   ...
   Begin VB.CommandButton CToExcel 
      Height          =   525
      Left            =   3240
      Picture         =   "FPurchaseReport.frx":2010B0
      Style           =   1  'Graphical
      TabIndex        =   7
      Top             =   6195
      Width           =   1365
   ...
   Begin VB.CommandButton CShowSummary 
      Height          =   505
      Left            =   390
      Picture         =   "FPurchaseReport.frx":203512
      Style           =   1  'Graphical
      TabIndex        =   5
      Top             =   6210
      Width           =   1350
   ...
   Begin VB.CommandButton CShowDetailed 
      Height          =   505
      Left            =   1830
      Picture         =   "FPurchaseReport.frx":205974
      Style           =   1  'Graphical
      TabIndex        =   6
      Top             =   6210
      Width           =   1350
   ...
   End
End
Attribute VB_Name = "FPurchaseReport"
...
Dim rs As Recordset
    ...
    Me.Caption = "Purchase Report - Detailed"
    ...
    If CoItem.ListIndex = -1 And CoSupplier.ListIndex = -1 Then
        If CoManufacturer.ListIndex = -1 Then
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Supplier,Tax,Transaction.Quantity,Transaction.PurchaseRate,ItemName From Transaction,ItemRegister Where (BillType = 'P' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (ItemRegister.Code=Transaction.ItemCode) Group By BillDate,BillNo,Supplier,Tax,Transaction.Quantity,Transaction.PurchaseRate,ItemName Order By BillDate,Val(BillNo)")
        Else
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Supplier,Tax,Transaction.Quantity,Transaction.PurchaseRate,ItemName From Transaction,ItemRegister Where (BillType = 'P' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "'))  And(Transaction.ItemCode In (Select ItemRegister.Code From ItemRegister Where(ItemRegister.ManufacturerCode='" & sManufacturerCode(CoManufacturer.ListIndex + 1) & "'))) And (ItemRegister.Code=Transaction.ItemCode) Group By BillDate,BillNo,Supplier,Tax,Transaction.Quantity,Transaction.PurchaseRate,ItemName Order By BillDate,Val(BillNo)")
        End If
    ElseIf CoItem.ListIndex = -1 And CoSupplier.ListIndex > -1 Then
        If CoManufacturer.ListIndex = -1 Then
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Supplier,Tax,Transaction.Quantity,Transaction.PurchaseRate,ItemName From Transaction,ItemRegister Where (BillType = 'P' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (ItemRegister.Code=Transaction.ItemCode) And (Transaction.SupplierCode ='" & sSupplierCode(CoSupplier.ListIndex + 1) & "') Group By BillDate,BillNo,Supplier,Tax,Transaction.Quantity,Transaction.PurchaseRate,ItemName Order By BillDate,Val(BillNo)")
        Else
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Supplier,Tax,Transaction.Quantity,Transaction.PurchaseRate,ItemName From Transaction,ItemRegister Where (BillType = 'P' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And(Transaction.ItemCode In (Select ItemRegister.Code From ItemRegister Where(ItemRegister.ManufacturerCode='" & sManufacturerCode(CoManufacturer.ListIndex + 1) & "'))) And (Transaction.SupplierCode ='" & sSupplierCode(CoSupplier.ListIndex + 1) & "') And (ItemRegister.Code=Transaction.ItemCode) Group By BillDate,BillNo,Supplier,Tax,Transaction.Quantity,Transaction.PurchaseRate,ItemName Order By BillDate,Val(BillNo)")
        End If
    ElseIf CoItem.ListIndex > -1 And CoSupplier.ListIndex = -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Supplier,Tax,Transaction.Quantity,Transaction.PurchaseRate,ItemName From Transaction,ItemRegister Where (BillType = 'P' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "'))  And(Transaction.ItemCode ='" & sItemCode(CoItem.ListIndex + 1) & "') And (ItemRegister.Code=Transaction.ItemCode) Group By BillDate,BillNo,Supplier,Tax,Transaction.Quantity,Transaction.PurchaseRate,ItemName Order By BillDate,Val(BillNo)")
    ElseIf CoItem.ListIndex > -1 And CoSupplier.ListIndex > -1 Then
...
Dim rs As Recordset
    ...
    Me.Caption = "Purchase Report - Summary"
    
    Dim dAdvance, dDiscount, dExtraCharge, dTaxAmount, dBasicAmount, dBillAmount, dBalance As Double
    
    If CoItem.ListIndex = -1 And CoSupplier.ListIndex = -1 Then
        If CoManufacturer.ListIndex = -1 Then
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Supplier,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.PurchaseRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.PurchaseRate) As TotalAmount From Transaction Where (BillType = 'P' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) Group By BillDate,BillNo,Supplier,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
        Else
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Supplier,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.PurchaseRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.PurchaseRate) As TotalAmount From Transaction Where (BillType = 'P' )And(Transaction.ItemCode In (Select ItemRegister.Code From ItemRegister Where(ItemRegister.ManufacturerCode='" & sManufacturerCode(CoManufacturer.ListIndex + 1) & "'))) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) Group By BillDate,BillNo,Supplier,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
        End If
    ElseIf CoItem.ListIndex = -1 And CoSupplier.ListIndex > -1 Then
        If CoManufacturer.ListIndex = -1 Then
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Supplier,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.PurchaseRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.PurchaseRate) As TotalAmount From Transaction Where (BillType = 'P' ) And(SupplierCode ='" & sSupplierCode(CoSupplier.ListIndex + 1) & "') And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) Group By BillDate,BillNo,Supplier,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
        Else
            Set rs = db.OpenRecordset("Select BillDate,BillNo,Supplier,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.PurchaseRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.PurchaseRate) As TotalAmount From Transaction Where (BillType = 'P' ) And(Transaction.ItemCode In (Select ItemRegister.Code From ItemRegister Where(ItemRegister.ManufacturerCode='" & sManufacturerCode(CoManufacturer.ListIndex + 1) & "'))) And (Transaction.SupplierCode ='" & sSupplierCode(CoSupplier.ListIndex + 1) & "') And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) Group By BillDate,BillNo,Supplier,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
        End If
    ElseIf CoItem.ListIndex > -1 And CoSupplier.ListIndex = -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Supplier,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.PurchaseRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.PurchaseRate) As TotalAmount From Transaction Where (BillType = 'P' )And(Transaction.ItemCode ='" & sItemCode(CoItem.ListIndex + 1) & "') And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) Group By BillDate,BillNo,Supplier,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
    ElseIf CoItem.ListIndex > -1 And CoSupplier.ListIndex > -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Supplier,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.PurchaseRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.PurchaseRate) As TotalAmount From Transaction Where (BillType = 'P' ) And(Transaction.ItemCode ='" & sItemCode(CoItem.ListIndex + 1) & "') And (Transaction.SupplierCode ='" & sSupplierCode(CoSupplier.ListIndex + 1) & "') And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) Group By BillDate,BillNo,Supplier,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
    End If
...
Private Sub MGrid_DblClick()
    If MGrid.Rows > 0 Then
        FPurchase.Show
        FPurchase.TTransactionNo = MGrid.TextMatrix(MGrid.Row, 1)
