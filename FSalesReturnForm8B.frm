...
Begin VB.Form FSalesReturnForm8B 
   BackColor       =   &H00EFEFEF&
   ...
   ClientWidth     =   14700
   ControlBox      =   0   'False
   Icon            =   "FSalesReturnForm8B.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MinButton       =   0   'False
   Picture         =   "FSalesReturnForm8B.frx":628A
   ScaleHeight     =   9255
   ScaleWidth      =   14700
   ...
   Begin VB.CommandButton CAddItem 
      Height          =   500
      Left            =   300
      Picture         =   "FSalesReturnForm8B.frx":204ECC
      Style           =   1  'Graphical
      TabIndex        =   13
      Top             =   7770
      Width           =   1365
   ...
   Begin VB.CommandButton CRemoveItem 
      Height          =   500
      Left            =   1740
      Picture         =   "FSalesReturnForm8B.frx":20732E
      Style           =   1  'Graphical
      TabIndex        =   14
      Top             =   7770
      Width           =   1365
   ...
   Begin VB.CommandButton CClear 
      Height          =   500
      Left            =   3180
      Picture         =   "FSalesReturnForm8B.frx":209790
      Style           =   1  'Graphical
      TabIndex        =   15
      Top             =   7770
      Width           =   1365
   ...
   Begin VB.CommandButton CNew 
      Height          =   500
      Left            =   315
      Picture         =   "FSalesReturnForm8B.frx":20BBF2
      Style           =   1  'Graphical
      TabIndex        =   20
      Top             =   8670
      Width           =   1365
   ...
   Begin VB.CommandButton CSave 
      Height          =   500
      Left            =   10755
      Picture         =   "FSalesReturnForm8B.frx":20E054
      Style           =   1  'Graphical
      TabIndex        =   19
      Top             =   8670
      Width           =   1365
   ...
   Begin VB.CommandButton CClose 
      ...
      Height          =   500
      Left            =   12195
      Picture         =   "FSalesReturnForm8B.frx":2104B6
      Style           =   1  'Graphical
      TabIndex        =   22
      Top             =   8670
      Width           =   1365
   ...
   Begin VB.CommandButton CDelete 
      Height          =   500
      Left            =   4530
      Picture         =   "FSalesReturnForm8B.frx":212918
      Style           =   1  'Graphical
      TabIndex        =   21
      Top             =   135
      Width           =   1365
   ...
   Begin MSForms.Label Label2 
      ...
      BackColor       =   15724527
      Size            =   "25638;926"
      Picture         =   "FSalesReturnForm8B.frx":214D7A
      FontHeight      =   165
      FontCharSet     =   0
      FontPitchAndFamily=   2
   End
End
Attribute VB_Name = "FSalesReturnForm8B"
...
Dim gSerialNo As Single, gItem As Single, gQuantity As Single, gMRP As Single, gTaxAmount As Single, gTax As Single, gItemDiscount As Single, gGrossValue As Single, gNetValue As Single, gUnit As Single, gSaleRate As Single, gRetail As Single, gWholeSale As Single, gOther As Single, gTotalAmount As Single, gBillingName As Single, gItemCode As Single, gPurchaseRate As Single, gTaxedValue As Single
...
Dim rs As Recordset, sTransactionNo As String
    ...
    Set rs = db.OpenRecordset("Select Max(Val( Transaction.BillNo)) As TNo From Transaction Where ( Transaction.BillType = 'SBR' ) And (Transaction.FinancialCode='" & getFinancialCode(DTPDate.Value) & "')")
...
Dim rs As Recordset, r As Long, i As Long, isFinished As Boolean, dSaleRate As Double
    ...
    Set rs = db.OpenRecordset("Select Sum(Transaction.Quantity*Transaction.UnitValue) As Quantity,Transaction.PurchaseRate,Transaction.MRP,Transaction.Retail,Transaction.WholeSale,Transaction.Other From Transaction Where (Transaction.BillType = ('SBR') ) And (Transaction.ItemCode = '" & sItemCode(CoItem.ListIndex + 1) & "' ) And (Transaction.BillNo = '" & Trim(TTransactionNo.Text) & "') Group By Transaction.PurchaseRate,Transaction.MRP,Transaction.Retail,Transaction.WholeSale,Transaction.Other")
    While rs.EOF = False
        r = 0
...
Dim rs As Recordset, lYN As Long, bFound As Boolean
    ...
    If (MsgBox("Do you want to Delete the Bill ?", vbDefaultButton2 Or vbYesNo) = vbYes) Then
        Set rs = db.OpenRecordset("Select Transaction.* From Transaction Where (Transaction.BillNo = '" & Trim(TTransactionNo.Text) & "' ) And (Transaction.BillType = 'SBR' ) And (Transaction.FinancialCode='" & getFinancialCode(DTPDate.Value) & "')")
        While rs.EOF = False
            bFound = True
            rs.Delete
            rs.MoveNext
...
Dim rs As Recordset, sTransactionNo As String, LSerialNo As Long
    ...
    If (Val(LGrandAmount.Caption) + Val(TExtraCharge.Text)) > 0 Then
         ...
         rs!Credit = 0
         rs!Narration = "Sales Return Bill No" & TTransactionNo.Text & " " & Trim(TNarration.Text)
         rs!AddedBy = sCurrentUserCode
         rs!EditedBy = sCurrentUserCode
    ...
    If Val(TAdvance.Text) > 0 Then
        ...
        rs!Credit = 0
        rs!Narration = "Sales Return Form 8B Advance Bill No" & TTransactionNo.Text & " " & Trim(TNarration.Text)
        rs!AddedBy = sCurrentUserCode
        rs!EditedBy = sCurrentUserCode
        rs!SerialNo = LSerialNo
        rs!FinancialCode = getFinancialCode(DTPDate.Value)
    ...
    If Val(TDiscount.Text) > 0 Then
        ...
        rs!Credit = 0
        rs!Narration = "Sales Return Form 8B Disount Bill No" & TTransactionNo.Text & " " & Trim(TNarration.Text)
        rs!AddedBy = sCurrentUserCode
        rs!EditedBy = sCurrentUserCode
        rs!SerialNo = LSerialNo
        rs!FinancialCode = getFinancialCode(DTPDate.Value)
        ...
        rs!InventoryType = "SBR"
        rs!GCode = getGCodeOfAccount(IIf(CoCustomer.ListIndex <> -1, sAccountCode(CoCustomer.ListIndex + 1), sGeneralCustomerAccountID))
        rs!CreditedDebitedTo = "Sales Return Form 8B Discount"
        rs!Mode = "Credit"
        rs.Update
...
Dim rs As Recordset, r As Long
    ...
    Set rs = db.OpenRecordset("Select ItemRegister.ItemName,ItemRegister.BillingName,ItemRegister.Code,Units.UnitName,Transaction.*,Manufacturer.ShortName,(Select IM.ItemName From ItemRegister AS IM Where(IM.Code=ItemRegister.GroupCode)) As GroupName From ItemRegister,Transaction,Units,Manufacturer Where (Transaction.BillNo = '" & Trim(TTransactionNo.Text) & "' ) And (Transaction.BillType = 'SBR' ) And (ItemRegister.Code = Transaction.ItemCode ) And (Units.Code = ItemRegister.SaleUnitCode ) And (Manufacturer.Code=ItemRegister.ManufacturerCode) And (Transaction.FinancialCode='" & getFinancialCode(DTPDate.Value) & "') Order By Transaction.SerialNo")
    MGrid.Rows = 0
    If rs.RecordCount > 0 Then
...
Dim rs As Recordset, r As Long
    ...
    Set rs = db.OpenRecordset("Select ItemRegister.ItemName,ItemRegister.BillingName,ItemRegister.Code,Units.UnitName,Transaction.*,Manufacturer.ShortName,(Select IM.ItemName From ItemRegister AS IM Where(IM.Code=ItemRegister.GroupCode)) As GroupName From ItemRegister,Transaction,Units,Manufacturer Where (Transaction.BillNo = '" & Trim(TRefNo.Text) & "' ) And (Transaction.BillType = 'SB' ) And (ItemRegister.Code = Transaction.ItemCode ) And (Units.Code = ItemRegister.SaleUnitCode ) And (Manufacturer.Code=ItemRegister.ManufacturerCode) And (Transaction.FinancialCode='" & getFinancialCode(DTPDate.Value) & "') Order By Transaction.SerialNo")
    MGrid.Rows = 0
    If rs.RecordCount > 0 Then
