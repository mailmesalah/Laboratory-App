...
Begin VB.Form FSalesForm8BReturnReport 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Sales Form 8B Return Report"
   ClientHeight    =   6810
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   15270
   ...
   EndProperty
   Icon            =   "FSalesForm8BReturnReport.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MinButton       =   0   'False
   Picture         =   "FSalesForm8BReturnReport.frx":000C
   ScaleHeight     =   6810
   ScaleWidth      =   15270
   ...
   Begin VB.CommandButton CShowDetailed 
      Height          =   505
      Left            =   1815
      Picture         =   "FSalesForm8BReturnReport.frx":1FEC4E
      Style           =   1  'Graphical
      TabIndex        =   4
      Top             =   6150
      Width           =   1350
   ...
   Begin VB.CommandButton CShowSummary 
      Height          =   505
      Left            =   375
      Picture         =   "FSalesForm8BReturnReport.frx":2010B0
      Style           =   1  'Graphical
      TabIndex        =   3
      Top             =   6150
      Width           =   1350
   ...
   Begin VB.CommandButton CToExcel 
      Height          =   525
      Left            =   3225
      Picture         =   "FSalesForm8BReturnReport.frx":203512
      Style           =   1  'Graphical
      TabIndex        =   5
      Top             =   6135
      Width           =   1365
   ...
   Begin VB.CommandButton CClose 
      ...
      Height          =   505
      Left            =   13485
      Picture         =   "FSalesForm8BReturnReport.frx":205974
      Style           =   1  'Graphical
      TabIndex        =   6
      Top             =   6120
      Width           =   1365
   ...
   End
End
Attribute VB_Name = "FSalesForm8BReturnReport"
...
Dim rs As Recordset
    ...
    Me.Caption = "Sales Form 8B Return Report - Detailed"
    
    Dim dQuantity, dGrossValue, dTaxAmount, dTotalAmount As Double

    'Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName From Transaction,ItemRegister Where (BillType = 'SBR' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (ItemRegister.Code=Transaction.ItemCode) Group By BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName Order By BillDate,Val(BillNo)")
    If CoItem.ListIndex = -1 And CoCustomer.ListIndex = -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName From Transaction,ItemRegister Where (BillType = 'SBR' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (ItemRegister.Code=Transaction.ItemCode) Group By BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName Order By BillDate,Val(BillNo)")
    ElseIf CoItem.ListIndex = -1 And CoCustomer.ListIndex > -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName From Transaction,ItemRegister Where (BillType = 'SBR' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (ItemRegister.Code=Transaction.ItemCode) And (Transaction.CustomerCode ='" & sCustomerCode(CoCustomer.ListIndex + 1) & "') Group By BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName Order By BillDate,Val(BillNo)")
    ElseIf CoItem.ListIndex > -1 And CoCustomer.ListIndex = -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName From Transaction,ItemRegister Where (BillType = 'SBR' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "')) And (ItemRegister.Code=Transaction.ItemCode) And (Transaction.ItemCode ='" & sItemCode(CoItem.ListIndex + 1) & "') Group By BillDate,BillNo,Customer,Tax,Transaction.Quantity,Transaction.SaleRate,ItemName Order By BillDate,Val(BillNo)")
    ElseIf CoItem.ListIndex > -1 And CoCustomer.ListIndex > -1 Then
...
Dim rs As Recordset
    ...
    Me.Caption = "Sales Form 8B Return Report - Summary"
    
    Dim dAdvance, dDiscount, dExtraCharge, dTaxAmount, dBasicAmount, dBillAmount, dBalance As Double
    
    'Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges,Tax,Sum(Abs(Transaction.Quantity)*Transaction.SaleRate) As TotalAmount From Transaction Where (BillType = 'SBR' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) Group By BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges,Tax Order By BillDate,Val(BillNo)")
    If CoItem.ListIndex = -1 And CoCustomer.ListIndex = -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.SaleRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.SaleRate) As TotalAmount From Transaction Where (BillType = 'SBR' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) Group By BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
    ElseIf CoItem.ListIndex = -1 And CoCustomer.ListIndex > -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.SaleRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.SaleRate) As TotalAmount From Transaction Where (BillType = 'SBR' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) And (Transaction.CustomerCode ='" & sCustomerCode(CoCustomer.ListIndex + 1) & "') Group By BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
    ElseIf CoItem.ListIndex > -1 And CoCustomer.ListIndex = -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.SaleRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.SaleRate) As TotalAmount From Transaction Where (BillType = 'SBR' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) And (Transaction.ItemCode ='" & sItemCode(CoItem.ListIndex + 1) & "') Group By BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
    ElseIf CoItem.ListIndex > -1 And CoCustomer.ListIndex > -1 Then
        Set rs = db.OpenRecordset("Select BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges,Sum((Abs(Transaction.Quantity)*Transaction.SaleRate)*(Transaction.Tax/100)) As TaxAmount,Sum(Abs(Transaction.Quantity)*Transaction.SaleRate) As TotalAmount From Transaction Where (BillType = 'SBR' ) And (BillDate Between cDate('" & DTPFrom.Value & "') And cDate('" & DTPTo.Value & "') ) And (Transaction.ItemCode ='" & sItemCode(CoItem.ListIndex + 1) & "') And (Transaction.CustomerCode ='" & sCustomerCode(CoCustomer.ListIndex + 1) & "') Group By BillDate,BillNo,Customer,Narration,Advance,Discount,ExtraCharges Order By BillDate,Val(BillNo)")
    End If
...
Private Sub MGrid_DblClick()
    If MGrid.Rows > 0 Then
        ...
        FSalesReturnForm8B.TTransactionNo = MGrid.TextMatrix(MGrid.Row, 1)
